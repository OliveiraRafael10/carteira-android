...
public class LancamentoDAO {
    ...
    public double buscarTotalPorTipo(String tipo) {
        double total = 0.0;
        Cursor cursor = db.rawQuery(
                "SELECT SUM(" + DbHelper.COLUMN_VALOR + ") FROM " + DbHelper.TABLE_LANCAMENTOS_NAME + " WHERE " + DbHelper.COLUMN_TIPO + " = ?",
                new String[]{tipo.toUpperCase(Locale.ROOT)}
        );
